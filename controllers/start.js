...
module.exports = function (db, send) {
  ...
  const action = async function (_user, _message) {
    ...
    if (_message && _message.phone_number)
      set = { phone: _message.phone_number };
    ...
    if (!user) {
      if (!set) {
        send(
          ...
          messages.phone_required_keyboard
        );
      ...
      const tags = await db.Tag.find({
        recommendee: set.name ? set.name : set.phone,
      });

      user = await db.User.create(
        _message && _message.phone_number
          ? {
              ..._user,
              phone: _message.phone_number,
              tags: tags
                .map((tag) => tag.tag)
                .filter(unique)
                .join(", "),
